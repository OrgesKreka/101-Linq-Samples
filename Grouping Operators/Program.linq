...
public class LinqSamples
{
	...
	public LinqSamples()
	{
		_testDs = TestHelper.CreateTestDataSet();
	}
	...
	public void Sample40()
	{
		...
		// using inline method style
		/*
		var tmp = numbers.GroupBy(n => n.Field<int>("number") % 5)
						 .Select(g => new {
						 	Remainder = g.Key,
							Numbers = g
						 });
	...
	public void Sample41()
	{
		...
		// inline method style
		/*
		var tmp = words4.GroupBy(x => x.Field<string>("word")[0])
						.Select(g => new {
							FirstLetter = g.Key,
							Words = g
						}).Dump();
	...
	public void Sample42()
	{
		...
		// inline method style
		/*
		products.GroupBy(x => x.Field<string>("Category"))
				.Select(t => new
				{
					Category = t.Key,
					Products = t	
		...
		foreach (var cog in customerOrderGroups)
		{
			...
			foreach(var yg in cog.YearGroups)
			{
				...
				foreach(var mg in yg.MonthGroups)
				{
					...
					{
						Console.WriteLine($"\t\t\t OrderId= {order.Field<int>("OrderID")}");
	...
	private class AnagramEqualityComparer : IEqualityComparer<string>
	{
		...
		{
			var wordsChar = word.ToArray();
			Array.Sort<char>(wordsChar);
	...
	{
		var anagrams = _testDs.Tables["Anagrams"].AsEnumerable();
